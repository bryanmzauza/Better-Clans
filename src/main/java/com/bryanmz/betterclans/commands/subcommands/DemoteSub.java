...
public final class DemoteSub extends AbstractSub {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length < 1) { sender.sendMessage(plugin.messages().get("general.usage", "usage", "/clan demote <jogador>")); return; }
        ...
        if (tm.role() != ClanRole.OFFICER) {
            sender.sendMessage(plugin.messages().get("clan.demote.already", "player", args[0]));
            return;
        }
        tm.setRole(ClanRole.MEMBER);
